-- =====================================================================
--  FILE 16 : Custom task name per schedule
--
--  A schedule can now name its generated tasks — "NCPL Daily stock entry"
--  instead of the master's generic "Bookkeeping / Data Entry". Blank keeps
--  the master's name. Applies to future generation only.
--
...
begin
  ...
  loop
    ...
    insert into public.tasks (
      ...
      coalesce(nullif(btrim(r.custom_title), ''), r.name),
      r.task_master_id, r.client_id, r.assigned_to,
...
-- Verify after setting a custom name on a schedule and generating:
--   select title, period from public.tasks order by created_at desc limit 3;
